 ...
 * Return Value : .
 *****************************************************************************/
#define MAILBOX_PER_GROUP (8)
#define BYTES_PER_MAILBOX_GROUP (0x400)
...
static void r_canfd_mb_read (uint32_t buffer, can_frame_t * const frame)
{
    ...
    uint8_t mailbox = buffer % MAILBOX_PER_GROUP;
    uint8_t mailbox_group = buffer / MAILBOX_PER_GROUP;

    /* Get pointer to message buffer (FIFOs use the same buffer structure) */
    volatile canfd_rmb_t *p_mb_regs =
        (is_mb) ? (volatile canfd_rmb_t *)((unsigned char *)(&CANFD.RMB0) + mailbox_group * BYTES_PER_MAILBOX_GROUP + mailbox * sizeof(canfd_rmb_t))
